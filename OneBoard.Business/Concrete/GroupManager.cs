...
{
    public class GroupManager : BaseService<IGroupDal,Group>, IGroupService
    {
        ...
        public IDataResult<Group> GetById(int Id)
        {
            try
            {
                return new SuccessDataResult<Group>(_dal.Find(Id),
        ...
        #region async
        public async Task<IResult> AddAsync(Group entity)
        {
            try
            {
                ...
                return new SuccessDataResult<Group>(entity, BasicCrudOperationMessages.SUCCESS_ADD);
            }
            catch (Exception e)
            {
        ...
        public async Task<IResult> DeleteByIdAsync(int Id)
        {
            try
            {
                Group entity = await _dal.FindByIdAsync(Id);
                if (entity == null)
                {
                    return new FailDataResult<Group>($"{BasicCrudOperationMessages.NOT_FOUND_ENTİTY} :That belonging to {Id} any entity.");
                }
                else
                {
        ...
        public async Task<IDataResult<Group>> FindByIdAsync(int Id)
        {
            try
            {
                Group entity = await _dal.FindByIdAsync(Id);
                ...
                return new SuccessDataResult<Group>(entity, $"{BasicCrudOperationMessages.SUCCESS_GET_ID}");
            }
            catch (Exception ex)
            {
                return new FailDataResult<Group>($"{BasicCrudOperationMessages.FIND_BY_ID_ERROR} :Exception : {ex.Message}");
            }
        }

        public async Task<IDataResult<IList<Group>>> GetEntityValuesAsync()
        {
            try
            {
                //filling entity list
                IEnumerable<Group> entities = await _dal.ListAsync();
                return new SuccessDataResult<IList<Group>>(entities.ToList(), $"{BasicCrudOperationMessages.SUCCESS_GET_LİST}");
            }
            catch (Exception ex)
            {

                return new FailDataResult<IList<Group>>($"There is an error.When it finding the list : {ex.Message}");
            }
        }

        public async Task<IResult> UpdateAsync(Group entity)
        {
            try
            {
